...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.block = nn.Sequential(nn.MaxPool3d(kernel_size=2), DoubleConv(in_channels,out_channels))
...
class UNet3D(nn.Module):
    def __init__(self):
        ...
        self.bottleneck = Down(filters*8,filters*16)
        ...
        self.outc = nn.Conv3d(filters,OUT_CLASSES,kernel_size=1)
...
if __name__ == "__main__":
    ...
    print(model)
    x = torch.randn(1, IN_CHANNELS, 128, 128, 128).to(device)
